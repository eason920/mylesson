...
學生版的「英語聊天室」:00 ~ :50
只刷在 55~15 & 25~45 (一般排程中)不另在 16~24的空檔時加刷 
=> 16~24 的聊天室滿客狀態不是即時的
-->
...
<html lang="en">
	<head>
		...
		<script src="./2020/js/cpn_3_1100810.js?<%=Timer%>"></script>
		<!---->
...
<script>
	const vueBulletin = new Vue({
		created(){
			const vm = this;
			console.log('%cIS PAGE3+英語聊天室','color:yellow');
					...
					success(res){
						...
						if( updateMinutes == ':30' ){
							...
							if( l2 != 0 && l%max > 0 ){
								const addNum = max - (l%max);
								for( i=0;i<addNum;i++ ){
									vm.timeBlock[indexZero].ary.push({empty: true});
								};
							}
						};
						// HELF ONLY ^
