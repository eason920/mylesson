...
const renderFn = function(DATA){
	// $.getJSON('./2020/data/level_basic.json?'+today, function(levelData){
		$.getJSON('./data/level_basic.json?'+today, function(levelData){
		// =============================
		// == RADAR : FUNCTION
		// =============================
		...
		let memAry = [];
		const cutEnd = DATA.radar.level.indexOf('-');
		const level = DATA.radar.level.slice(0, cutEnd);
		...
		for( let a in levelData[level].group ){
			...
			const memNum = DATA.radar.data[index];
