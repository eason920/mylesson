...
$(()=>{
	...
	const fnFaceDataRecording = function(){
		...
		// -----------------------------------
		// -- ID 退位前，確認是否要在退週後補物件 v
		// -----------------------------------
		const firstWeek = $('.ui-datepicker-group-first tbody .ui-datepicker-week-col:eq(0)').text();
		const firstWeekFirstDate = $('.ui-datepicker-group-first tbody td:eq(1) ').text();
		let firstWeekAry;
		let firstDailyDoneAry = [];
		let beAdd = false;
		console.log( String(viewYear) + viewMonth);
		console.log('%c' + (String(viewYear) == 2016 ) + ( viewMonth == 2), 'color: yellow');
		if (viewWeek == firstWeek && firstWeekFirstDate != 1 ){ 
			beAdd = true;
			firstWeekAry = viewWeekAry;
			...
			if (viewYear == 2016 && viewMonth == 2 ){
				console.log( firstWeekAry );
			}
		}
